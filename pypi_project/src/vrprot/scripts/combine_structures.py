...
def main(directory: str, target: str, subprocess: bool, processing: str, color:list,image = False):
...
if __name__ == "ChimeraX_sandbox_1":
    ...
    parser.add_argument("directory", help="Directory that contains all PDB files from the bulk download.")
    parser.add_argument("target",help="Directory where the combined structures should be saved.")
    parser.add_argument("--subprocess","-sp", help="If this is set ChimeraX will close after all structures are done.", default=False,action="store_true")
    parser.add_argument("--processing_mode","-mode", help="The processing mode that should be applied to the structures.", default="cartoons_ss_coloring")
